...
export class ReviewForm implements OnInit {
  ...
  ngOnInit(): void {
    ...
    // ✅ Get phone + user_id from PhoneEntry component navigation
    this.route.queryParams.subscribe(params => {
      this.phone = params['phone'];
      this.user_id = +params['user_id']; // convert to number
    });
  }
  ...
  onSubmit(): void {
    if (this.reviewForm.invalid || !this.user_id) return;

    this.isLoading = true;
    const { rating, comment } = this.reviewForm.value;

    this.reviewService.submitReview(this.user_id, environment.businessId, rating, comment).subscribe({
      next: () => {
        this.isLoading = false;
        ...
        // ✅ go to mini-game with same query params
        this.router.navigate(['mini-game'], { queryParams: { phone: this.phone, user_id: this.user_id } });
      },
      error: () => {
        this.isLoading = false;
...
export class ReviewFormComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    // ✅ Get phone + user_id from PhoneEntry navigation
    this.route.queryParams.subscribe(params => {
      this.phone = params['phone'];
      this.user_id = +params['user_id']; // convert to number
    });
  ...
  onSubmit(): void {
    ...
    this.reviewService.submitReview(this.user_id, environment.businessId, rating, comment).subscribe({
      next: () => {
        ...
        if (rating >= 4) {
          this.snackBar.open('Thanks! Redirecting to Google Reviews...', 'Close', { duration: 3000 });
          window.open(environment.googleReviewUrl, '_blank'); // ✅ High rating
        } else {
          this.snackBar.open('Your review will be sent to admin for approval.', 'Close', { duration: 3000 });
        }

        // ✅ Always continue to mini-game
        this.router.navigate(['mini-game'], { queryParams: { phone: this.phone, user_id: this.user_id } });
      },
      error: (err) => {
        this.isLoading = false;
        this.snackBar.open('Failed to submit review. ' + (err.error?.error || 'Please try again.'), 'Close', { duration: 3000 });
      }
    });
  }
}*/
/*
// This main and important file for production but due to click button it not works but it is final code 
import { Component, OnInit } from '@angular/core';
import { CommonModule } from '@angular/common';
import { FormBuilder, FormGroup, Validators, ReactiveFormsModule } from '@angular/forms';
import { Router, ActivatedRoute } from '@angular/router';
...
export class ReviewForm implements OnInit {
  ...
  ngOnInit(): void {
    ...
    // ✅ Get phone + user_id from PhoneEntry component
    this.route.queryParams.subscribe(params => {
      this.phone = params['phone'];
  ...
  onSubmit(): void {
    ...
    this.reviewService.submitReview(this.user_id, environment.businessId, rating, comment).subscribe({
      next: () => {
        ...
        if (rating >= 4) { 
          this.snackBar.open('Thanks! Redirecting to Google Reviews...', 'Close', { duration: 3000 });
          this.reviewService.openGoogleReview(comment);
        } else {
          this.snackBar.open('Your review will be sent for admin approval.', 'Close', { duration: 3000 });
        }

        // ✅ Always continue to mini-game
        this.router.navigate(['spin'], { queryParams: { phone: this.phone, user_id: this.user_id } });
      },
      /*error: () => {
        this.isLoading = false;
        this.snackBar.open('Failed to submit review. Please try again.', 'Close', { duration: 3000 });
  ...
  onSubmit(): void {
    ...
    const payload: SubmitReviewPayload = {
      business_id: environment.businessId,
      rating,
      comment
    };
    ...
    this.reviewService.submitReview(payload).subscribe({
      next: (res: SubmitReviewResponse) => {
        this.isLoading = false;
  
        if (res.data?.google_review_link) {
          this.snackBar.open('Thanks! Redirecting to Google Reviews...', 'Close', { duration: 3000 });
          window.open(res.data.google_review_link, '_blank');
        } else if (res.data?.spin_page_url) {
          this.snackBar.open('Thanks! You can now play the mini-game.', 'Close', { duration: 3000 });
        } else if (res.message) {
          this.snackBar.open(res.message, 'Close', { duration: 3000 });
        }
  
        if (res.data?.spin_page_url) {
          this.router.navigate([res.data.spin_page_url], {
            queryParams: { phone: this.phone, user_id: this.user_id }
          });
        }
      },
      error: () => {
...
import { MatIconModule } from '@angular/material/icon';
import { MatProgressSpinnerModule } from '@angular/material/progress-spinner';
import { ReviewService, SubmitReviewPayload, SubmitReviewResponse } from '../../services/review';
import { environment } from '../../../environments/environment';
...
export class ReviewForm implements OnInit {
  ...
  phone!: string;
  user_id!: number;
  userName!: string; // optional: can use actual name
  stars = [1, 2, 3, 4, 5];
  isLoading = false;
  ...
  ngOnInit(): void {
    ...
    // Get phone, user_id, and optionally userName from query params
    this.route.queryParams.subscribe(params => {
      this.phone = params['phone'];
      this.user_id = +params['user_id'];
  ...
  onSubmit(): void {
    ...
    if (this.user_id) {
      ...
      payload.phone = this.phone;
      payload.name = this.userName || 'Guest';
    }
